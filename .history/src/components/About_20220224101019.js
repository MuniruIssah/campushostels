import React from "react";

const About = () => {
  return (
    <div className="bg-gray-900 flex flex-col items-center justify-center space-y-5 md:space-y-10 lg:p-20 p-10 w-screen">
      <h1 className="hidden md:block tracking-tight  font-extrabold text-3xl text-white sm:text-4xl md:text-5xl">
        About Campus Hostels
      </h1>
      <div className="p-6 bg-gray-800 w-full shadow-md md:w-4/5 min-h-[10rem] flex justify-center items-center rounded rounded-lg">
        <p className="md:text-center text-gray-500 text-lg font-semibold md:w-4/5 mt-3 text-base text-gray-500 sm:mt-5 sm:text-lg sm:max-w-xl sm:mx-auto md:mt-5 md:text-xl lg:mx-0">
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Repellat
          recusandae consectetur alias cum nihil doloribus unde sapiente a,
          aliquam reprehenderit laudantium nam earum ab autem itaque nulla.
          Sequi, consequuntur cumque.
        </p>
      </div>
    </div>
  );
};

export default About;
