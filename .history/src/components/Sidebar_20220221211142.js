  ...
  return (
    <div className="hidden md:block basis-1/4  h-full bg-slate-200 flex flex-col py-4 px-2">
      ...
      <div className="h-4/5 overflow-y-auto">
        {
            hostels.map((hostel,index)=><HostelItem key={index} {...hostel} />)
        }
      </div>
    </div>
  );
...
const HostelItem = ({
  ...
  return <div className="h-52 flex shadow shadow-2xl bg-white">
      <img src={images[1]?images[1]:images[0]} alt="" className="w-full h-32 object-cover hover:contrast-50" />
  </div>;
};
