...
const HostelListItem = ({ name, images }) => {
  return (
    <div className="group w-full md:w-auto mx-2 my-2 bg-white rounded py-4 px-4 font-semibold text-gray-800 mr-5 relative  hover:bg-indigo-600 hover:text-white flex space-x-3 items-center">
      <div className="hidden group-hover:block absolute -top-1 -right-1  bg-indigo-500 rounded rounded-full p-1 shadow-lg">
        <FiEdit2 className="text-xs " />
      </div>
      <div className="w-12">
        <img src={images[1] ? images[1] : images[0]} alt="" className="object-cover rounded rounded-full" />
      </div>
      <span>{name}</span>
    </div>
  );
